$(document).ready(function(){
    ...
    /*var data = 
        [{
            ...
            nombre: {original: '',modificado: 0,cambio: 0}
        },{ 
    ...
    function retornaDatos(id,url) {
        return $.ajax({
            ...
            success: function(arreglo){;
                ...
                $('#latitudEditarZona').val(arreglo.nombre);
                $('#longitudEditarZona').val(arreglo.nombre);
            }
    ...
    $('.editarProyecto').click(function(){
        ...
        var url = devuelveUrl('html/cliente/datosProyecto.php');
        var id = $(this).attr('id');
        var datos = retornaDatos(id,url);
        datos.success(function(respuesta){
            data[0].idEmpresa = respuesta.idEmpresa;
            data[1].idProyecto = respuesta.idProyecto;
            data[2].nombre.original = respuesta.nombre;
        });
    });
    $('.modalEditarProyecto').on('click','#btnEditarProyecto',function(){
        borrarMensajes();
        ...
        if(arreglo.length == 0) {
            ...
            if(flag != true){
                var url = devuelveUrl('html/cliente/editarProyecto.php');
                $.ajax({
                    url : url,
                    type: 'POST',
                    data:{datos: data },
