<?php
    ...
    if(isset($_SESSION['datos'])) { // si no existe la variable session redirigo a login
        ...
        if($_SESSION['datos']['tipoUsuario'] == 'Cliente') { // si la sesión es de un cliente puede continuar con la ejecución
            ...
            if(isset($_GET['idResultado']) and isset($_GET['patente']) and isset($_GET['idArchivo']) and isset($_GET['fechaDatos'])) {// si existen las variables
                if(verifyIdIntPositive($_GET['idResultado']) and strlen($_GET['patente']) == 6 and verifyIdIntPositive($_GET['idArchivo']) and verifyDateFormat($_GET['fechaDatos'])) {
                    ...
                    $nombreZona = nombreZona($idResultado);
...
<body>
    ...
    <?php
        echo '<div class="fix2 center2"><i class="fa fa-globe" style="color: #F5A214;"></i>'.$nombreZona['nombreZona'].'<i class="fa fa-" style="color: #F5A214;"></i><i class="fa fa-calendar" style="color: #F5A214;"></i>'.$fecha.'<i class="fa fa-cog" style="color: #F5A214;"></i>'.$patente.'</div>';
    ?>
